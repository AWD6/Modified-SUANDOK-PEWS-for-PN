// --- 1. ข้อมูลกลุ่มอายุ ---
const ageGroups = [
    { id: 'newborn',    name: 'Newborn',    ageRange: 'แรกเกิด-1 เดือน',        heartRate: { min: 120, max: 160 }, respiratoryRate: { min: 30, max: 60 } },
    ...
    { id: 'adolescent', name: 'Adolescent', ageRange: '13-15 ปี',               heartRate: { min: 60,  max: 100 }, respiratoryRate: { min: 20, max: 30 } }
];
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // CHANGE: reset now calls resetForm() + scrolls to top instead of reloading
...
function calculateRespiratoryScore() {
    ...
    if (!isNaN(spo2)) {
        if (state.chdType === 'cyanotic') {
            ...
            let threshold = 95;
            if (state.termCondition === 'preterm') threshold = 90;
            spo2Score = spo2 < threshold ? 3 : 0;
        }
    }
...
async function submitToGoogleForm(record) {
    ...
    const conditionStr = record.chdType
        ? chdTypeMapping[record.chdType]
        : (record.termCondition === 'preterm' ? 'Preterm' : (record.termCondition === 'term' ? 'Term' : 'ไม่ระบุ'));
...
async function saveRecord(action) {
    ...
    const record = {
        ...
        termCondition: state.termCondition,
        ...
        return `
            <div class="record-card">
                ...
                <div class="record-details">
                    ...
                    ${record.palsEnabled ? `<div class="detail-row"><span class="detail-label">CTAS:</span><span class="pals-badge">CTAS</span></div>` : ''}
                </div>
